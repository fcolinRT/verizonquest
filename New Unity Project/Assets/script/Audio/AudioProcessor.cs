...
public class AudioProcessor : MonoBehaviour
{
    ...
    public StructEvents[] eventsToProcess;
    ...
    private void initArrays()
    {
        ...
        alphs = new float[eventsToProcess.Length];
        for (int i = 0; i < alphs.Length; i++)
            alphs[i] = 100 * eventsToProcess[i].gThresh;
        //alph = 100 * gThresh;
    }
    ...
    void Update()
    {
        if (audioSource.isPlaying)
        {
            ...
            for (int j = 0; j < eventsToProcess.Length; j++)
            {
                ...
                //            alph = 100 * gThresh;
                //for (int i = 0; i < alphs.Length; i++)
                alphs[j] = 100 * eventsToProcess[j].gThresh;
                ...
                for (int i = tempopd / 2; i < System.Math.Min(colmax, 2 * tempopd); ++i)
                {
                    ...
                    float score = onset + scorefun[(now - i + colmax) % colmax] - alphs[j] * (float)System.Math.Pow(System.Math.Log((float)i / (float)tempopd), 2);
                ...
                if (smaxix == now)
                {
                    ...
                    {
                        // onBeat.Invoke();
                        eventsToProcess[j].onBeat.Invoke();
                        blipDelay[0] = 1;
